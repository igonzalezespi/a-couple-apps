...
Keep secrets out of source and config: real values live only in a gitignored `.env`, the committed `.env.example` ships placeholders, env parsing fails fast on missing required values, and secret scanning runs in the pipeline.
