...
Define the testing strategy: Vitest with `@testing-library/react` (rendering React Native primitives through a react-native -> react-native-web alias) for unit/component tests, plus cross-platform end-to-end coverage (Maestro for native, Playwright for the React Native Web build), runnable locally and wired into CI.
